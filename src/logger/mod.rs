...
pub fn log(mut nes: NES) {
    println!(
        "{: <6}{: <10}{: <32}{}",
        program_counter_log(nes.cpu_registers.program_counter).blue(),
        instruction_log(&mut nes).cyan(),
        assembly_log(&mut nes).yellow(),
        cpu_registers_log(&mut nes).magenta()
    );
}
...
fn instruction_log(nes: &mut NES) -> String {
    let opcode = nes.cpu_peek(nes.cpu_registers.program_counter);
    let instruction = &cpu::instructions::INSTRUCTIONS_TABLE[opcode as usize];

    let log = match instruction.len {
        1 => format!("{:02X}", opcode),
        2 => {
            let operand = nes.cpu_peek(nes.cpu_registers.program_counter + 1);
            format!("{:02X} {:02X}", opcode, operand)
        }
        3 => {
            let operand1 = nes.cpu_peek(nes.cpu_registers.program_counter + 1);
            let operand2 = nes.cpu_peek(nes.cpu_registers.program_counter + 2);
...
fn assembly_log(nes: &mut NES) -> String {
    ...
    let (mem_addr, stored) = match instruction.mode {
        ...
        _ => {
            ...
            let stored = nes.cpu_peek(addr);

            (addr, stored)
        }
    };

    let addr = nes.cpu_peek(program_counter + 1);
    let addr_16 = nes.cpu_peek_u16(program_counter + 1);
    ...
    match instruction.mode {
        ...
        AddrMode::Indirect => {
            let jump_addr = if addr_16 & 0x00FF == 0x00FF {
                let low = nes.cpu_peek(addr_16);
                let high = nes.cpu_peek(addr_16 & 0xFF00);

                u16::from_le_bytes([low, high])
            } else {
                nes.cpu_peek_u16(addr_16)
            };
...
fn cpu_registers_log(nes: &mut NES) -> String {
    let ppu_cycle_log = ppu_cycle_log(nes);
    ...
    format!(
        ...
        ppu_cycle_log,
        nes.cpu_total_cycles,
    )
}

fn ppu_cycle_log(nes: &mut NES) -> String {
    let ppu_cycle = nes.ppu_cycle;
    let ppu_scanline = nes.ppu_scanline;

    format!("{: >3},{: >3}", ppu_scanline, ppu_cycle)
}
